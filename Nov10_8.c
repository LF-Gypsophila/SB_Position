...
typedef struct {
    ...
    bool     isOJT[MAX_EMP];                 // ★ 追加：各従業員がOJTかどうか
} Schedule;
...
static void make_half_break_pairs(Schedule* sch, int h){
    ...
    while (assigned < want){
        ...
        if (j == -1){
            ...
            // こうすることで、後半の専門席（OS 等）を i が肩代わりして欠けさせる事故を防ぐ
...
static void build_schedule(Schedule* sch, Employee emps[], int empCount){
    ...
    for (int h = 0; h < sch->hours; ++h){
        ...
        (void)filled; // 未使用抑止（必要なら充足チェックで使える）
    ...
    for (int h = 0; h < sch->hours; ++h){
        for (int i = 0; i < sch->empCount; ++i){
            ...
            if (tag == 'F' && h > 0){
                ...
                // ★ OJTが禁止ポジションに流れた場合のガード（理論上ここでは起きにくいが安全策）
...
int main(void){
    ...
    // ★ OJT設定：true を OJT とする（店舗に合わせてここを編集）
    //   例：Mika と Ken を OJT 扱い
    for (int i = 0; i < empCount; ++i) sch.isOJT[i] = false;
    sch.isOJT[2] = true; // Mika
    sch.isOJT[3] = true; // Ken
