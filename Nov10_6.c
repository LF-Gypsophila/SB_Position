...
static int build_joblist_for_hour(Position joblist[], int maxJobs, const int need[POS_COUNT]){
    // BREAK は “半休憩” の表現を出力で行うため、ここでは席に含めない
...
static void make_half_break_pairs(Schedule* sch, int h){
    ...
    while (assigned < want){
        ...
        // ポジション調整：
        // - どちらかが TRAIN の場合は実ポジション側をコピーして、前半/後半ともそのポジションがカバーされるようにする
        // - 両方が実ポジションを持つ場合は swap して「交代」させる（前半は j が、後半は i がそれぞれのポジションを担当）
        Position pos_i = sch->assign[h][i];
        Position pos_j = sch->assign[h][j];

        if (pos_i == TRAIN && pos_j != TRAIN){
            // i が TRAIN、j が実ポジション → i に j のポジションをコピー（両半分でそのポジションがカバーされる）
            sch->assign[h][i] = pos_j;
            // sch->assign[h][j] は pos_j のまま（前半担当）
        } else if (pos_j == TRAIN && pos_i != TRAIN){
            // j が TRAIN、i が実ポジション → j に i のポジションをコピー
            sch->assign[h][j] = pos_i;
            // sch->assign[h][i] は pos_i のまま（後半担当）
            // 注意：i は 'F'（後半担当）、j は 'L'（前半担当）なので前半/後半で pos_i がカバーされる
        } else {
            // 双方とも実ポジション（または双方TRAIN）→ swap（交代）
